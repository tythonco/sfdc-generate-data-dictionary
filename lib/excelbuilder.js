...
module.exports = class Downloader {
  ...
  writeFields(worksheet, fields, line, validationRules) {
    ...
    for (var j = 0; j < fields.length; j++) {
      ...
      if (this.config.hiddenPrefixList.length > 0) {
        let prefixMatched = false;
        this.config.hiddenPrefixList.forEach(function(prefix) {
          if (field.name.toLowerCase().startsWith(prefix.toLowerCase())) {
            prefixMatched = true;
          }
        });
        if (prefixMatched) {
  ...
  generate() {
    const promise = new Promise((resolve, reject) => {
      ...
      for (var i = 0; i < sObjects.length; i++) {
        ...
        var metadataPath = describePath;

        if (fs.existsSync(describePath)) {
          var currentObjectFieldsDescribe = JSON.parse(fs.readFileSync(describePath)).fields;
      ...
      var fileName = this.config.projectName !== ''
        ? this.config.projectName + '_Data_Dictionary_' + currentDateString + '.xlsx'
        : 'Data_Dictionary_' + currentDateString + '.xlsx'
